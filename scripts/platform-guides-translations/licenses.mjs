...
export default {
  ar: {
    ...
    body: String.raw`
      <p>البائعون ينشئون <strong>تراخيص</strong> ضد منتج وإصدار برمجيات، ثم يوزّعون <strong>مفتاح الترخيص</strong> و<code>license.bin</code> الموقّع. <em>نوع</em> الترخيص يحدد انتهاء الصلاحية والمقاعد والتفعيلات والجلسات وقواعد تحقق SDK.</p>
      ...
      <ul style="margin-left:1.25rem;">
        <li><strong>دائم</strong> — شراء مرة واحدة؛ online أو offline؛ انتهاء اختياري للصيانة.</li>
        <li><strong>تجريبي</strong> — تقييم محدود زمنياً؛ تمديد أو تحويل إلى مدفوع.</li>
        <li><strong>اشتراك</strong> — مدة متكررة؛ تجديد أو إلغاء عند مغادرة.</li>
        <li><strong>عائم</strong> — مجموعة مشتركة؛ المقاعد تُفرض بـ <em>الجلسات الحية</em>؛ دائماً online.</li>
        <li><strong>متزامن</strong> — مفتاح فريق مشترك؛ المقاعد تُفرض بـ <em>تفعيلات الأجهزة</em>.</li>
        <li><strong>مقفل على جهاز</strong> — آلة واحدة لكل ترخيص؛ ربط عتاد + نقل؛ ربط تلقائي أو موافقة المالك؛ محاولات المشاركة من آلات أخرى تُسجّل وتُبلّغ المالك.</li>
        <li><strong>قائم على الرصيد</strong> (<code>MeteredToken</code>) — online فقط؛ محفظة رصيد مشتركة؛ تسعير رمز لكل ميزة. \${link('/credit-based-license', 'الدليل الكامل')}.</li>
        <li><strong>قائم على الاستخدام</strong> (<code>MeteredCount</code>) — online فقط؛ عدّاد استخدام مشترك؛ استخدام واحد لكل consume. \${link('/usage-based-license', 'الدليل الكامل')}.</li>
      </ul>
      ...
      <ol style="margin-left:1.25rem;">
        ...
        <li><strong>تشغيل</strong> — مراقبة التفعيلات/الجلسات؛ تمديد التجارب؛ تعديل المقاعد حيث مسموح.</li>
        <li><strong>إلغاء / حذف</strong> — إبطال فوري لمسارات online؛ ملفات offline تفشل عند التحديث online التالي.</li>
      ...
      \${screenshot('platform-licenses-list.png', 'قائمة Licenses — أعمدة Type وMode وStatus وActivations (مثل 2/∞ دائم، 1/1 node-locked)')}

      <h2>معالج الإنشاء — الخطوة 1: Activation</h2>
      <p>اختر كيف يُتحقق من الترخيص وقت التشغيل:</p>
      <ul style="margin-left:1.25rem;">
        <li><strong>Online</strong> — تحقق الخادم والتفعيلات والجلسات؛ مطلوب للعائم والاشتراك وقائم على الرصيد وقائم على الاستخدام.</li>
        <li><strong>Offline</strong> — <code>license.bin</code> الموقّع فقط؛ الدائم والتجريبي وNode-Locked يمكنهم استخدام offline.</li>
      </ul>
      ...
      <ul style="margin-left:1.25rem;">
        <li><strong>بطاقات النوع</strong> — Perpetual، Trial، Subscription، Floating، Concurrent، Node-Locked، <strong>Credit-Based</strong>، <strong>Usage-Based</strong> (الخطة قد تحد الأنواع).</li>
      ...
      <ul style="margin-left:1.25rem;">
        <li><strong>Product</strong> — مطلوب عند الإنشاء من القائمة العامة؛ مُعبّأ مسبقاً في سياق المنتج.</li>
      ...
      <h3>Activations</h3>
      <p>مقاعد الجهاز والبيئة لنماذج الربط. مرئية عندما يدعم نوع الترخيص تتبع العتاد/الجهاز ولديك حقوق owner/Admin.</p>
      <ul style="margin-left:1.25rem;">
        <li>صفوف لكل جهاز مع block/unblock وملاحظات الدعم.</li>
        <li>السطح الأساسي لحساب مقاعد \${link('/concurrent-license', 'Concurrent')} (تفعيلات، وليس جلسات).</li>
      ...
      <h3>Sessions</h3>
      <p>اتصالات online حية مع طوابع نبضات القلب. تُعرض للتراخيص القادرة على online — خاصة \${link('/floating-license', 'Floating')} حيث المقاعد تتبع <em>الجلسات الحية</em>، ولـ \${link('/perpetual-license', 'Perpetual online')} / \${link('/node-locked-license', 'Node-Locked online')} للمراقبة.</p>
      <ul style="margin-left:1.25rem;">
      ...
      \${screenshot('platform-node-locked-sessions-tab.png', 'تفاصيل الترخيص — علامة Active Sessions مع نبضات القلب وقطع الاتصال')}
      ...
      <h3>Device</h3>
      <p>تراخيص <strong>Node-locked</strong> فقط: بصمة العتاد المرتبطة، سجل التفعيل، سير عمل النقل/فك الربط عندما تسمح السياسة بتغيير الجهاز، وبطاقة <strong>Unauthorized Access Attempts</strong> تسرد الآلات المرفوضة لأن الترخيص مرتبط في مكان آخر. المحاولات تحمل معرّفات العتاد المحاولة والمرتبط وIP ونقطة الدخول وعدّاد لكل آلة؛ مالكو المنتج يُبلّغون داخل التطبيق مرة واحدة لكل آلة مخالفة يومياً ويمكنهم وضع علامة reviewed على كل واحدة. راجع \${link('/node-locked-license', 'كشف مشاركة ترخيص node-locked')}.</p>

      <h2>دورة حياة حالة الترخيص</h2>
      <ul style="margin-left:1.25rem;">
        <li><strong>Active</strong> — صالح للتحقق ضمن حدود السياسة.</li>
        <li><strong>Expired</strong> — بعد انتهاء الصلاحية (تجريبي/اشتراك)؛ التحقق يفشل.</li>
        <li><strong>Revoked</strong> — إيقاف بقرار المشغّل؛ التحقق online يفشل فوراً.</li>
      ...
      <ul style="margin-left:1.25rem;">
        ...
        <li><strong>Member معيّن</strong> — قد يرى التراخيص على الإصدارات المسموحة؛ عادةً لا يمكنه الإنشاء.</li>
      ...
      <p>\${link('/api/licenses', 'API التراخيص')} · \${link('/sdk/license-client', 'SDK LicenseClient')} · \${link('/guides/platform/products', 'المنتجات')}</p>
    `,
  },
  fr: {
    title: 'Licences (espace éditeur)',
    lead:
      "Émettre, distribuer, révoquer et surveiller les licences depuis le tableau de bord. Couvre l'assistant création en quatre étapes, onglets détail (dont Usage pour types mesurés), types de licence et vues éditeur vs utilisateur final.",
    body: String.raw`
      ...
      <p>Parcours : \${link('/first-license', 'Générer votre première licence')} · Guides modèles : \${link('/perpetual-license', 'Perpétuel')}, \${link('/trial-license', 'Essai')}, \${link('/subscription-license', 'Abonnement')}, \${link('/floating-license', 'Flottant')}, \${link('/concurrent-license', 'Concurrent')}, \${link('/node-locked-license', 'Nœud verrouillé')}, \${link('/credit-based-license', 'Basé crédits')}, \${link('/usage-based-license', 'Basé usage')}.</p>

      <h2>Modèles licence en un coup d'œil</h2>
      <ul style="margin-left:1.25rem;">
        <li><strong>Perpétuel</strong> — achat unique ; online ou offline ; expiration optionnelle maintenance.</li>
        <li><strong>Essai</strong> — évaluation limitée dans le temps ; prolonger ou convertir en payant.</li>
        <li><strong>Abonnement</strong> — terme récurrent ; renouveler ou révoquer au churn.</li>
        <li><strong>Flottant</strong> — pool partagé ; sièges appliqués par <em>sessions live</em> ; toujours online.</li>
        <li><strong>Concurrent</strong> — clé équipe partagée ; sièges appliqués par <em>activations appareil</em>.</li>
        <li><strong>Nœud verrouillé</strong> — une machine par licence ; liaison matérielle + transfert ; auto-bind ou approbation propriétaire ; tentatives partage depuis autres machines enregistrées et signalées au propriétaire.</li>
        <li><strong>Basé crédits</strong> (<code>MeteredToken</code>) — online uniquement ; portefeuille crédits partagé ; tarification token par feature. \${link('/credit-based-license', 'Guide complet')}.</li>
        <li><strong>Basé usage</strong> (<code>MeteredCount</code>) — online uniquement ; compteur usage partagé ; un usage par consume. \${link('/usage-based-license', 'Guide complet')}.</li>
      </ul>
      ...
      <ol style="margin-left:1.25rem;">
        ...
        <li><strong>Exploiter</strong> — surveiller activations/sessions ; prolonger essais ; ajuster sièges si autorisé.</li>
        <li><strong>Révoquer / supprimer</strong> — invalidation immédiate chemins online ; fichiers offline échouent au prochain refresh online.</li>
      ...
      <p>Utilisez liste sidebar <strong>Licenses</strong> pour recherche cross-produit, ou ouvrez produit → onglet <strong>Licenses</strong> quand le contexte logiciel est connu. Clients finaux utilisent <strong>My Licenses</strong> (pas espace éditeur) pour clés émises à leur compte.</p>
      \${screenshot('platform-licenses-list.png', 'Liste Licenses — colonnes Type, Mode, Status et Activations (p.ex. 2/∞ perpétuel, 1/1 node-locked)')}

      <h2>Assistant création — Étape 1 : Activation</h2>
      <p>Choisissez comment la licence est validée à runtime :</p>
      <ul style="margin-left:1.25rem;">
        <li><strong>Online</strong> — validation serveur, activations et sessions ; requis pour Flottant, Abonnement, Basé crédits et Basé usage.</li>
        <li><strong>Offline</strong> — <code>license.bin</code> signé uniquement ; Perpétuel, Essai et Node-Locked peuvent utiliser offline.</li>
      </ul>
      ...
      <ul style="margin-left:1.25rem;">
        <li><strong>Cartes type</strong> — Perpetual, Trial, Subscription, Floating, Concurrent, Node-Locked, <strong>Credit-Based</strong>, <strong>Usage-Based</strong> (forfait peut limiter types).</li>
      ...
      <ul style="margin-left:1.25rem;">
        ...
        <li><strong>Issuer</strong> — nom éditeur optionnel dans métadonnées licence.</li>
      ...
      <p>Limites opérationnelles et libellés utilisés par opérateurs et support après émission.</p>
      <ul style="margin-left:1.25rem;">
      ...
      <p>Résumé mono-volet pour ops éditeur quotidiennes.</p>
      <ul style="margin-left:1.25rem;">
        ...
        <li>Copier clé pour livraison email ; audit log corrèle téléchargements et IP.</li>
      </ul>

      <h3>Activations</h3>
      <p>Sièges appareil et environnement pour modèles liaison. Visible quand le type licence supporte suivi matériel/appareil et droits owner/Admin.</p>
      <ul style="margin-left:1.25rem;">
      ...
      <h3>Sessions</h3>
      <p>Connexions online live avec horodatages heartbeat. Affichées pour licences online-capable — surtout \${link('/floating-license', 'Flottant')} où sièges suivent <em>sessions live</em>, et pour \${link('/perpetual-license', 'Perpetual online')} / \${link('/node-locked-license', 'Node-Locked online')} monitoring.</p>
      <ul style="margin-left:1.25rem;">
      ...
      \${screenshot('platform-node-locked-sessions-tab.png', 'Détail licence — onglet Active Sessions avec heartbeat et disconnect')}
      ...
      <h3>Device</h3>
      <p>Licences <strong>Node-locked</strong> uniquement : empreinte matérielle liée, historique activation, workflows transfert/unbind quand politique autorise changement machine, et carte <strong>Unauthorized Access Attempts</strong> listant machines refusées car licence liée ailleurs. Tentatives portent IDs matériel tenté et lié, IP, point d'entrée et compteur par machine ; propriétaires produit notifiés in-app une fois par machine fautive par jour et peuvent marquer reviewed. Voir \${link('/node-locked-license', 'Détection partage licence node-locked')}.</p>

      <h2>Cycle de vie statut licence</h2>
      <ul style="margin-left:1.25rem;">
        <li><strong>Active</strong> — valide pour validation dans limites politique.</li>
        <li><strong>Expired</strong> — après expiration (essai/abonnement) ; validation échoue.</li>
        <li><strong>Revoked</strong> — arrêt forcé opérateur ; validation online échoue immédiatement.</li>
        <li><strong>Suspended / locked</strong> — suspensions facturation ou conformité (selon déploiement).</li>
      </ul>

      <h2>Rôles</h2>
      <ul style="margin-left:1.25rem;">
        <li><strong>Propriétaire produit / system Admin</strong> — créer, modifier, révoquer, supprimer, voir onglet Activations.</li>
        <li><strong>Member assigné</strong> — peut voir licences sur versions permises ; création généralement interdite.</li>
        <li><strong>Utilisateur final</strong> — routé vers <code>/dashboard/my-licenses</code> pour clés émises à son compte.</li>
      </ul>
